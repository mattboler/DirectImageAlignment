...
if lvl == 1
   ...
   return;
    ...
    % Two options:
    %{
    Average inverse depth (in matlab this automatically sets all invalid
    points to depth of 0)
    %}
    I_lvl = ( (I_tl.^(-1) + I_tr.^(-1) + I_bl.^(-1) + I_br.^(-1)) ./ (sign(I_tl) + sign(I_tr) + sign(I_bl) + sign(I_br)) ).^(-1);
    
    %{
    Average depth and then manually handle nan answers:
